...
func (sr *SnapshotRepository) createDistribution(publicationHref string, repoConfigUUID string, snapshotId string) (string, string, error) {
	...
	distTask, err := sr.pulpClient.PollTask(*distTaskHref)
	if err != nil {
		return "", "", err
	}
...
func (sr *SnapshotRepository) UpdatePayload() error {
	// json, err := json.Marshal(*sr.payload)
	// if err != nil {
	// 	return err
	// }
	// sr.task.Payload = json
	var err error
	a := *sr.payload
	sr.task, err = (*sr.queue).UpdatePayload(sr.ctx, sr.task, a)
	if err != nil {
		return err
	}
	return nil
